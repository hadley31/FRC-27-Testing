...
public class AutoTrajectory {
  ...
  final Mechanism driveSubsystem;
  ...
  public Command cmd() {
    if (trajectory.samples().isEmpty() && warnUser) {
      return driveSubsystem.run(coroutine -> noSamples.addCause(name)).named("Trajectory_" + name);
    }
    return driveSubsystem
        .run(
            coroutine -> {
              cmdInitialize();
              // Execute at least once before checking for completion, matching the v2 scheduler's
              // execute-then-isFinished ordering.
              while (true) {
                cmdExecute();
                if (cmdIsFinished()) {
                  break;
  ...
  public Command spawnCmd() {
    return Command.noRequirements(coroutine -> coroutine.fork(cmd()))
        .named("Trajectory_" + name + "_Spawner");
  }
  ...
  public Trigger active() {
    return routine.active().and(new Trigger(Scheduler.getDefault(), routine.loop(), () -> this.isActive));
  }
  ...
  public Trigger doneDelayed(double seconds) {
    return timeTrigger(seconds, inactiveTimer)
        .and(new Trigger(Scheduler.getDefault(), routine.loop(), () -> isCompleted));
  }
  ...
  public Trigger atTime(double timeSinceStart) {
    ...
    if (timeSinceStart < 0) {
      ...
      return new Trigger(Scheduler.getDefault(), routine.loop(), () -> false);
    }
  ...
  public Trigger atTime(String eventName) {
    ...
    Trigger trig = new Trigger(Scheduler.getDefault(), routine.loop(), () -> false);
