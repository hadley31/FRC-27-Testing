...
public class AutoRoutine {
  ...
  private final Trigger isActiveTrigger = new Trigger(Scheduler.getDefault(), loop,
      () -> isActive && RobotState.isEnabled());

  /** A boolean indicating if a trajectory is running on the routine right now */
  private boolean isIdle = true;

  private final Trigger isIdleTrigger = new Trigger(Scheduler.getDefault(), loop, () -> isIdle);
  ...
  public Trigger observe(BooleanSupplier condition) {
    return new Trigger(Scheduler.getDefault(), loop, condition);
  }
  ...
  public Command cmd(BooleanSupplier finishCondition) {
    return Command.noRequirements(
        coroutine -> {
          ...
          // Poll at least once before checking for completion, matching the v2 scheduler's
          // execute-then-isFinished ordering.
          while (!RobotState.isDisabled() && !finishCondition.getAsBoolean()) {
            poll();
            coroutine.yield();
          }
          // whenCanceled only covers cancellation, so reset explicitly on natural completion too.
          reset();
        })
        .whenCanceled(this::reset)
        .named(name);
  }
}
