...
public class AutoFactory {
  ...
  private final Mechanism driveSubsystem;
   ...
   * @param driveSubsystem The drive {@link Mechanism} to require for {@link AutoTrajectory} {@link
   *     Command}s.
   ...
   * </code></pre>
   *
   * <p>Whether this command can actually run while disabled depends on whether {@code
   * driveSubsystem} is {@link Mechanism#controllableDuringDisabled() controllable during disabled}.
   *
  ...
  public Command warmupCmd() {
    ...
    Command warmup = autoTraj.cmd().withTimeout(Seconds.of(0.5));
    return Command.noRequirements(coroutine -> coroutine.await(warmup))
        .named("Choreo Warmup Command");
  }
  ...
  public Command resetOdometry(Supplier<Optional<Pose2d>> pose) {
    return driveSubsystem
        .run(coroutine -> pose.get().ifPresent(resetOdometry))
        .named("ResetOdometry");
  }
