...
public class AutoChooser implements ComplexTunable {
  ...
  public AutoChooser(String doNothingName) {
    ...
    generatedCommand = none(DO_NOTHING_NAME);
    addCmd(DO_NOTHING_NAME, () -> none(DO_NOTHING_NAME));
  ...
  private static Command renamed(Command inner, String name) {
    return Command.noRequirements(coroutine -> coroutine.await(inner)).named(name);
  }
  ...
  private String select(String selectStr, boolean force) {
    ...
    if (dsValid || force) {
      ...
      generatedCommand = renamed(autoRoutines.get(nameAtGeneration).get(), nameAtGeneration);
    } else {
  ...
  public Command selectedCommandScheduler() {
    return Command.noRequirements(coroutine -> coroutine.await(selectedCommand()))
        .named("Selected Auto Routine");
  }
